...
def chatbot():
    ...
    if any(k in user_msg for k in ["price", "cost", "fee", "how much"]):
        responses = [
            "Our standard projects start at just â‚¹999. Custom projects are quoted individually based on your specific requirements.",
            "Standard UI/UX projects are usually â‚¹999-â‚¹1999. For custom dev, just submit an idea and Rakesh will set a price for you!",
...
def accept_custom(id):
    ...
    send_update_email(user.email, "Custom Project Accepted - ProjectMate", 
                      f"Hi {user.username}, your custom project request '{cp.title}' has been accepted with a price of â‚¹{cp.price}! Please pay the 50% upfront to start.")
    
    flash("Custom project accepted with price â‚¹" + str(cp.price) + "!", "success")
    return redirect("/admin")
